...
namespace Linearstar.Keystone.IO.Elfreina
{
    public class ElAnimationData
    {
        ...
        public IList<float> BoneAnimationTimeKeys { get; set; } = new List<float>();

        public IList<ElBoneAnimationPart> BoneAnimation { get; set; } = new List<ElBoneAnimationPart>();

        public IList<float> UVAnimationTimeKeys { get; set; } = new List<float>();

        public IList<ElUVAnimationPart> UVAnimation { get; set; } = new List<ElUVAnimationPart>();
            ...
            foreach (var i in data.Children)
                switch (i.Name)
                {
                    ...
                    case "Loop":
                        ...
                        break;
                    case "BoneAnimation":
                        rt.BoneAnimationTimeKeys = i.Child("TimeKeys").Children
                            .Select(_ => float.Parse(_.Values.First())).ToList();
                        rt.BoneAnimation = i.Children.Where(_ => _.Name == "AnimationPart")
                            .Select(ElBoneAnimationPart.Parse).ToList();

                        break;
                    case "UVAnimation":
                        rt.UVAnimationTimeKeys = i.Child("TimeKeys").Children.Select(_ => float.Parse(_.Values.First()))
                            .ToList();
                        rt.UVAnimation = i.Children.Where(_ => _.Name == "AnimationPart")
                            .Select(ElUVAnimationPart.Parse).ToList();
            ...
            if (this.BoneAnimation.Any())
                baseData.Child("BoneAnimation").Children =
                    this.BoneAnimation.Select(_ => _.ToData())
                        ...
                        {
                            Children = this.BoneAnimationTimeKeys
                                .Select(f => new ElData().SetValues(f.ToString("0.000000"))).ToList(),
                        }).ToList();
            else
            ...
            if (this.UVAnimation.Any())
                baseData.Child("UVAnimation").Children =
                    this.UVAnimation.Select(_ => _.ToData())
                        .StartWith(new ElData("TimeKeys")
                        {
                            Children = this.UVAnimationTimeKeys
                                .Select(f => new ElData().SetValues(f.ToString("0.000000"))).ToList(),
                        }).ToList();
            else
